// Memory utilities for Den Shell
// Shell-specific memory management structures
...
pub const LeakDetector = leak_detector.LeakDetector;
pub const LeakReport = leak_detector.LeakReport;
pub const DebugAllocator = leak_detector.DebugAllocator;

pub const ObjectPool = pool.ObjectPool;
pub const StringPool = pool.StringPool;
pub const SlabAllocator = pool.SlabAllocator;
...
/// Uses stack buffer for small strings, heap for larger ones
pub const StringBuilder = struct {
    ...
    pub fn init(alloc: std.mem.Allocator) StringBuilder {
        return .{
            .stack_buf = undefined,
            .heap_buf = null,
            .len = 0,
...
/// Groups allocations for a single command together
pub const CommandMemoryPool = struct {
    ...
    pub fn init(alloc: std.mem.Allocator) CommandMemoryPool {
        return .{
            .allocator = alloc,
